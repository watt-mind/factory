...
/** A read gets its initial request plus at most two rate-limit retries. */
const MAX_RATE_LIMIT_READ_ATTEMPTS = 3;
...
function rateLimitDelayMs(stderr, attempt, random) {
  const retryAfter = /retry-after\s*:\s*([^\s]+)/i.exec(stderr)?.[1];
  const seconds = Number(retryAfter);
  if (Number.isFinite(seconds) && seconds >= 0)
    return Math.min(RATE_LIMIT_BACKOFF_MAX_MS, Math.ceil(seconds * 1_000));
  const cap = Math.min(
    RATE_LIMIT_BACKOFF_MAX_MS,
    RATE_LIMIT_BACKOFF_MS * 2 ** Math.max(0, attempt - 1),
  );
  const sample = Math.min(1, Math.max(0, Number(random()) || 0));
  return Math.floor(cap / 2 + (sample * cap) / 2);
}

// A bare process exit code does not identify a rate limit: `gh` commonly
// exits 1 for every HTTP failure, and secondary limits are HTTP 403. Require
// GitHub's rate-limit language (or its unambiguous 429 response) instead.
function isRateLimited(stderr) {
  return /\b429\b|rate[ -]?limit|secondary|abuse/i.test(stderr);
}
...
export function githubForge({
  ...
  function gh(args, { cwd, timeout, retryOnRateLimit = false } = {}) {
    const budget = timeout === undefined ? defaultTimeout : timeout;
    for (let attempt = 1; attempt <= MAX_RATE_LIMIT_READ_ATTEMPTS; attempt++) {
      const r =
        exec("gh", args, {
          cwd,
          encoding: "utf8",
      ...
      const rateLimited = isRateLimited(stderr);
      const error = new ForgeError(
        ...
        { status, stdout, stderr, cause: r.error },
      );
      if (timedOut) error.code = "forge_timeout";
      else if (bufferExceeded) error.code = "forge_read_failed";
      else if (rateLimited) error.code = "forge_rate_limited";
      if (
        !retryOnRateLimit ||
        !rateLimited ||
        attempt === MAX_RATE_LIMIT_READ_ATTEMPTS
      )
        throw error;
      wait(rateLimitDelayMs(stderr, attempt, random));
    }
  }
  ...
  function resolveRepo(repo, opts) {
    ...
    const resolved = ghJson(["repo", "view", "--json", "nameWithOwner"], {
      ...opts,
  ...
  return {
    ...
    prView(repo, number, { fields, ...opts } = {}) {
      ...
      const readOpts = { ...opts, retryOnRateLimit: true };
      const pull = apiJson(`repos/${resolvedRepo}/pulls/${number}`, readOpts);
      const pr = restPullToPr(pull);
      if (fields?.includes("statusCheckRollup")) {
        pr.statusCheckRollup = statusCheckRollup(resolvedRepo, pull, readOpts);
      }
    ...
    prList(repo, { state, limit, fields, ...opts } = {}) {
      ...
      return prs.slice(0, requested).map((pull) => {
        ...
        const detailedPull = needsMergeability
          ? hydrateMergeability(resolvedRepo, pull, readOpts)
          : pull;
        const pr = restPullToPr(detailedPull);
        if (fields?.includes("statusCheckRollup")) {
          pr.statusCheckRollup = statusCheckRollup(
            resolvedRepo,
            pull,
            readOpts,
          );
    ...
    /**
     * Start a workflow run. Mutating, and deliberately fire-and-forget: the
     * dispatch API returns no run id, so callers poll for the run they just
     * asked for rather than being handed one that may not exist yet.
     */
    workflowDispatch(repo, workflow, { ref, inputs = {}, ...opts } = {}) {
      const args = ["workflow", "run", String(workflow), ...repoFlag(repo)];
    ...
    prCreate(repo, { base, head, title, body = "", ...opts } = {}) {
      if (!base || !head || !title) {
        throw new ForgeError("prCreate requires base, head and title", {
          status: 0,
        });
      }
      return gh(
    ...
    /**
     * `method` is explicit and has no default: a release PR merged with
     * `squash` destroys `git log master..develop` as the ship list, so the
     * caller states which it wants and an unknown value fails closed.
     *
     * `matchHeadCommit` is the SHA whose checks the caller verified. Between
     * reading a green check-run summary and merging, the branch can move; the
     * flag makes GitHub refuse rather than land an unverified commit.
     */
    prMerge(repo, number, { method, matchHeadCommit, ...opts } = {}) {
      const flag = { merge: "--merge", squash: "--squash", rebase: "--rebase" }[
        method
      ];
      if (!flag) {
        throw new ForgeError(
          `prMerge method ${JSON.stringify(method)} is not one of merge, squash, rebase`,
          { status: 0 },
        );
      }
      const args = ["pr", "merge", String(number), ...repoFlag(repo), flag];
      if (matchHeadCommit != null && matchHeadCommit !== "")
        args.push("--match-head-commit", String(matchHeadCommit));
      gh(args, opts);
    },
