 ...
 *   ref?: string,                      // branch or tag the run uses
 *   inputs?: Record<string, string>,   // workflow_dispatch inputs
 * }} WorkflowDispatchOpts
 */
...
/**
 * @typedef {ForgeCallOpts & { method: "merge"|"squash"|"rebase", matchHeadCommit?: string }} PrMergeOpts
 *   `matchHeadCommit` is the head SHA the caller verified. GitHub refuses the
 *   merge when the branch has moved since, which is the only way to close the
 *   window between "the checks on this commit are green" and "merge it".
 */
 ...
 *   `gh pr checks --watch`.
 * @property {(repo: RepoRef, workflow: string, opts?: WorkflowDispatchOpts) => void} workflowDispatch
 *   Start a workflow run. Returns nothing: the dispatch API hands back no run
 *   id, so a caller that needs the run polls {@link Forge.runList} for it.
 * @property {(repo: RepoRef, opts: PrCreateOpts) => string} prCreate
 *   Open a pull request; returns its URL.
 * @property {(repo: RepoRef, number: number|string, opts: PrMergeOpts) => void} prMerge
 *   Merge a pull request with the stated `method`. There is no default —
 *   squashing a release PR wrecks `git log <deploy>..<base>` as the ship list.
 *   With `matchHeadCommit` the merge fails rather than landing a commit whose
 *   checks the caller never saw.
