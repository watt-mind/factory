...
function fakeGhExec(seed) {
  const exec = (cmd, args) => {
    ...
    const restPull = (pr) => ({
      ...
      // Seeded PRs carry no base ref; the fixture default stands in for one.
      base: { ref: pr.baseRefName ?? "main" },
      mergeable: true,
    ...
    switch (`${positional[0]} ${positional[1]}`) {
      ...
      case "workflow run": {
        data.runs.unshift({
          databaseId: 900000 + data.runs.length,
          name: positional[2],
          workflowName: positional[2],
          path: `.github/workflows/${positional[2]}`,
          status: "in_progress",
          conclusion: null,
          headBranch: flags.ref ?? null,
          event: "workflow_dispatch",
        });
        return ok("");
      }
      case "pr create": {
        if (!flags.base || !flags.head || !flags.title)
          return fail("pull request create failed: missing required field");
        const number =
          data.prs.reduce(
            (max, pr) => Math.max(max, Number(pr.number) || 0),
            0,
          ) + 1;
      ...
      case "pr merge": {
        const pr = findPr(positional[2]);
        if (!pr) return fail("no pull requests found");
        const expected = flags["match-head-commit"];
        if (expected && String(pr.headRefOid ?? "") !== String(expected))
          return fail(
            "Pull Request is not mergeable: head commit did not match expected",
          );
        pr.state = "MERGED";
        pr.mergedAt ??= "2026-08-15T10:00:00Z";
        return ok("");
      }
      case "run list": {
        let runs = data.runs;
...
for (const [name, make] of IMPLEMENTATIONS) {
  describe(`forge contract: ${name}`, () => {
    ...
    test("workflowDispatch starts a run the caller can then find", () => {
      const { forge } = make();
      forge.workflowDispatch(REPO, "publish.yml", {
        ref: "develop",
        inputs: { commit_sha: "abc123" },
      });
      const started = forge
        .runList(REPO, { fields: ["workflowName", "status"] })
        .find((run) => run.workflowName === "publish.yml");
      expect(started).toEqual({
        workflowName: "publish.yml",
        status: "in_progress",
      });
      expect(() =>
        forge.workflowDispatch("nope/none", "publish.yml", {}),
      ).toThrow(ForgeError);
    });

    test("prCreate opens a PR and returns its URL; a missing field throws", () => {
      const { forge } = make();
      const url = forge.prCreate(REPO, {
        base: "master",
        head: "develop",
      ...
      expect(
        forge.prView(REPO, number, {
          fields: ["headRefName", "baseRefName", "title", "state"],
        }),
      ).toEqual({
        headRefName: "develop",
        baseRefName: "master",
      ...
      expect(() =>
        forge.prCreate(REPO, { base: "master", head: "develop" }),
      ).toThrow(ForgeError);
    });

    test("prMerge merges with the stated method and refuses an unknown one", () => {
      const { forge } = make();
      forge.prMerge(REPO, 1, { method: "merge" });
      expect(forge.prView(REPO, 1, { fields: ["state"] })).toEqual({
        state: "MERGED",
      });
      expect(() => forge.prMerge(REPO, 2, { method: "yolo" })).toThrow(
        ForgeError,
      );
      expect(() => forge.prMerge(REPO, 2, {})).toThrow(ForgeError);
      expect(() => forge.prMerge(REPO, 999, { method: "merge" })).toThrow(
        ForgeError,
      );
    });

    // WM-1104 finding 1: the window between "these checks are green" and
    // "merge it" is where a push lands an unverified commit on master.
    test("prMerge --match-head-commit refuses a head that moved, and merges the one it named", () => {
      const { forge, seed } = make();
      expect(() =>
        forge.prMerge(REPO, 1, {
          method: "merge",
      ...
      expect(seed.repos[REPO].prs[0].state).toBe("OPEN");

      forge.prMerge(REPO, 1, { method: "merge", matchHeadCommit: "abc123" });
      expect(forge.prView(REPO, 1, { fields: ["state"] })).toEqual({
        state: "MERGED",
      });
    });
...
describe("github forge: REST PR reads", () => {
  ...
  test("rate-limited prView retries idempotent reads, honors Retry-After, and stays typed", () => {
    let calls = 0;
    const waits = [];
    const forge = githubForge({
      exec: recorder(() => {
    ...
    expect(forge.prView("o/r", 7, { fields: ["state"] })).toEqual({
      state: "OPEN",
    });
    expect(calls).toBe(2);
    expect(waits).toEqual([2_000]);

    const nonIdempotent = githubForge({
      exec: recorder({
        status: 1,
        stdout: "",
    ...
    expect(() => nonIdempotent.prSetDraft("o/r", 7, true)).toThrow(
      expect.objectContaining({ code: "forge_rate_limited" }),
    );

    const ordinary403 = githubForge({
      exec: recorder({
        status: 1,
        stdout: "",
    ...
    expect(() => ordinary403.prList("o/r")).toThrow(
      expect.not.objectContaining({ code: "forge_rate_limited" }),
    );
  });
  ...
  test("prMerge emits --match-head-commit only when the caller names a SHA", () => {
    const exec = recorder({ status: 0, stdout: "", stderr: "" });
    const forge = githubForge({ exec });
    forge.prMerge("o/r", 7, { method: "merge", matchHeadCommit: "abc123" });
    ...
    forge.prMerge("o/r", 8, { method: "merge" });
    expect(exec.calls[1].args).not.toContain("--match-head-commit");
  });
