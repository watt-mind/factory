...
export function memoryForge({ repos = {}, api = {}, defaultRepo = null } = {}) {
  ...
  return {
    ...
    workflowDispatch(repo, workflow, { ref, inputs = {} } = {}) {
      calls.push({ op: "workflowDispatch", repo, workflow, ref, inputs });
      ...
      data.runs.unshift({
        ...
        conclusion: null,
        headBranch: ref ?? null,
        headSha: inputs.commit_sha ?? null,
        event: "workflow_dispatch",
      });
    },

    prCreate(repo, { base, head, title, body = "" } = {}) {
      calls.push({ op: "prCreate", repo, base, head, title, body });
      if (!base || !head || !title) {
        throw new ForgeError("prCreate requires base, head and title", {
          status: 1,
          stderr: "pull request create failed",
        });
      }
      const data = repoData(repo);
      const number =
        data.prs.reduce((max, pr) => Math.max(max, Number(pr.number) || 0), 0) +
        1;
      const url = `https://github.com/${repo ?? defaultRepo}/pull/${number}`;
      data.prs.push({
        number,
        state: "OPEN",
        isDraft: false,
    ...
    prMerge(repo, number, { method, matchHeadCommit } = {}) {
      calls.push({ op: "prMerge", repo, number, method, matchHeadCommit });
      if (!["merge", "squash", "rebase"].includes(method)) {
        throw new ForgeError(
          `prMerge method ${JSON.stringify(method)} is not one of merge, squash, rebase`,
          { status: 1, stderr: "unknown merge method" },
        );
      }
      const pr = findPr(repo, number);
      // Same refusal GitHub makes for `--match-head-commit`: the branch moved
      // since the caller read its checks, so this merge is not the one asked
      // for.
      if (
        matchHeadCommit != null &&
        matchHeadCommit !== "" &&
        String(pr.headRefOid ?? "") !== String(matchHeadCommit)
      ) {
        throw new ForgeError(
          `prMerge --match-head-commit ${matchHeadCommit} does not match #${number} head ${pr.headRefOid ?? "(none)"}`,
          {
            status: 1,
            stderr:
