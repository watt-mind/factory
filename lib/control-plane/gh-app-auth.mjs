 ...
 * file; live-stack uses the process which actually holds the lock as its
...
async function runLockedDaemon(config) {
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      stdio: "inherit",
      // flock forks for its command. Isolate its subtree so a signal received
      // by this wrapper can reach both flock and --daemon-held.
      detached: true,
    },
  );
  ...
  const forward = (signal) => {
    ...
    try {
      ...
      // The group can disappear between spawn and forwarding. Preserve the
      // direct-child behaviour for platforms/process states without it.
      child.kill(signal);
    }
  };
