...
describe("runDaemon", () => {
  ...
  test("SIGTERM stops the entire flock subtree and releases its lock", async () => {
    ...
    try {
      const deadline = Date.now() + 2_000;
      while (!existsSync(lockFile) && Date.now() < deadline) await Bun.sleep(5);
      expect(existsSync(lockFile)).toBe(true);

      daemon.kill("SIGTERM");
      await daemon.exited;

      const lockIsFree = Bun.spawnSync({
      ...
      const heldProcesses = Bun.spawnSync({
        cmd: ["ps", "-axo", "command="],
        stdout: "pipe",
        stderr: "pipe",
      })
        .stdout.toString()
        .split("\n")
        .filter((command) => command.includes(`${script} --daemon-held`));
