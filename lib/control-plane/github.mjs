  ...
  const spawn = (cmd, args, opts, { allowApp = true } = {}) => {
    ...
    try {
      ...
      if (token) {
        ...
        childEnv = allowApp ? { ...env, GH_TOKEN: token } : { ...env };
      }
    } catch (err) {
      if (!warnedAppAuth) {
        warnedAppAuth = true;
...
async function ghJson(exec, args, input) {
  ...
  if (status !== 0) {
    ...
    try {
      ...
      if (Number.isInteger(parsed?.status)) apiStatus = parsed.status;
    } catch {
...
export function githubControlPlane(options = {}) {
  ...
  function isPermissionFailure(error) {
    return (
      ...
      (error.status === 403 ||
        /resource not accessible by integration|forbidden|permission/i.test(
          error.message,
        ))
    );
  }
