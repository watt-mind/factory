...
export function githubControlPlane(options = {}) {
  ...
  return {
     ...
     * its current duplicate-title behaviour. A search miss with a dedupe key
     * also scans the latest 100 repository issues: GitHub's search index can
    ...
    async file({
      ...
      if (marker || titleToMatch) {
        ...
        // Search is eventually consistent. A retry immediately following a
        // post-create board failure must not create a second issue simply
        // because the original has not reached the search index yet.
        if (!created && marker) {
          const recent = await call("GET", `repos/${repo}/issues`, {
            query: { state: "all", per_page: "100" },
          });
          created = (Array.isArray(recent) ? recent : []).find(
            (issue) =>
              !issue?.pull_request &&
        ...
        reused = Boolean(created);
      }
      const warnings = [];
      if (reused && created.state === "closed")
        warnings.push(
          `reused closed issue #${created.number}: the dedupe key matched an issue that is already closed; reopen it or file under a new key if the finding is live`,
        );
      if (!created) {
      ...
      if (reused) {
        const currentNames = (created.labels ?? [])
          .map((label) => label?.name)
          .filter(Boolean);
        const missing = labels.filter((label) => !currentNames.includes(label));
        if (missing.length) {
          await call("PUT", `repos/${repo}/issues/${created.number}/labels`, {
            body: { labels: resolveLabelNames(currentNames, { add: missing }) },
          });
        }
      }
      const filed = {
...
export const PROTOCOL_LABELS = Object.freeze([
  ...
  [
    "has-screenshots",
    "1d76db",
    "This pull request has UX-review screenshot evidence",
  ],
  ["area:ui-ux", "c5def5", "User interface or user experience"],
