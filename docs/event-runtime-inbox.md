...
| Field         | Rule                                                                                                                                                                                                                                                                                                             |
| :------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `question`    | Required, 1–280 chars. The one thing being decided, phrased so one click answers it (`docs/protocol.md` §14 "one answerable sentence" rule, made structural).                                                                                                                                                    |
...
- **Bound to the ticket as written.** `descriptionHash` is
  ...
  with no added trailing newline. At worker gate time the runtime compares it
  to the current description and validates that `authorisation.paths` covers
  every parsed Owned Path. A mismatch refuses before the model runs with
  `authorisation_stale:description` or `authorisation_stale:paths`; a match adds
  runtime-owned `authorisation.verified: true` to the execution input. The
  runtime also checks at emit time and raises a fresh `ESCALATED` item instead
  of dispatching if the description already moved.
...
> operator has already seen the escalation. The worker compares the canonical
> `hashBytes` value (exact UTF-8 description bytes, no trailing newline) and
> authorised paths before the model runs, then sets
> `authorisation.verified: true`. Trust that runtime-owned flag and never
> recompute the hash. Proceed only when it is `true`, stay inside
> `authorisation.paths`, and quote the authorisation (item id, decided-at) in
> the PR body.
