...
| `runs.subscribe(cb)`                    | `cb(event)` for a lifecycle transition committed by *this* process. Process-local: a transition the worker (a separate process, OPS-233) commits never reaches it. Returns an unsubscribe function.        |
