...
def watchdog-sweep($runs) {
  ...
  # after any stack restart: `serve` runs its `reap` step every tick
  # (`event-runtime/cli/serve.mjs`), reclaiming expired leases after
  # LEASE_GRACE_SECONDS. Before an orphan is retried, preserve and push any
  # uncommitted worktree work; no reap tick runs while the stack is down.
}
```
...
| `GET /runs` list has no `spec` | The list exposes `runId`, `agent`, and `adapter`, but `row.spec` is absent                                     | Take ticket identity from `eventId` or `cli.mjs inspect <runId>`                                                                                                                            |
...
| Restart orphans in-flight      | `live-stack.sh down` can end a leaseholder; `serve` reaps expired leases each tick after `LEASE_GRACE_SECONDS` | Restart only when no `dispatch@1` run is RUNNING/LEASED; after `down`, push uncommitted worktree work, then let `event-runtime/cli/serve.mjs` reclaim expired leases after the grace period |
