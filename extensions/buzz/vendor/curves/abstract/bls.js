...
function createBlsPairing(fields, G1, G2, params) {
  ...
  if (typeof G1 !== "function")
    throw new TypeError(
      '"G1_Point" expected point constructor, got type=' + typeof G1,
    );
  if (typeof G2 !== "function")
    throw new TypeError(
      '"G2_Point" expected point constructor, got type=' + typeof G2,
    );
  validateObject(
    params,
  ...
  const mul014ByLine = ({ c0: f0, c1: f1 }, o0, l1, l4, Px, Py) => {
    ...
    const t0_c0 = add2(
      mul2ByNonresidue(sub2(mul2(add2(a1, a2), o1), t0_1)),
      t0_0,
    );
    ...
    const t2_c0 = add2(
      mul2ByNonresidue(sub2(mul2(add2(s1, s2), o14), t2_1)),
      t2_0,
    );
  ...
  const calcPairingPrecomputes = (point) => {
    if (!(point instanceof G2))
      throw new TypeError(
        '"point" expected G2 point, got type=' + typeof point,
      );
    const p = point;
  ...
  function pairingBatch(pairs, withFinalExponent = true) {
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      if (!(g1 instanceof G1))
        throw new TypeError(
          '"pairs[' + i + '].g1" expected G1 point, got type=' + typeof g1,
        );
      if (!(g2 instanceof G2))
        throw new TypeError(
          '"pairs[' + i + '].g2" expected G2 point, got type=' + typeof g2,
        );
  ...
  const rand =
    params.randomBytes === undefined ? randomBytes : params.randomBytes;
  ...
  return Object.freeze({
    ...
    getPublicKey(secretKey) {
      ...
      try {
        ...
        // @ts-ignore
        throw new Error("invalid private key: " + typeof secretKey, {
          cause: error,
        });
      }
