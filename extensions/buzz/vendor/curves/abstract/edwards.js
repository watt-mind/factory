...
export function edwards(params, extraOpts = {}) {
  ...
  class Point {
    ...
    add(other) {
      ...
      // E = (X1+Y1)*(X2+Y2)-A-B
      const E = Fp.sub(
        Fp.subN(Fp.mulN(Fp.addN(X1, Y1), Fp.addN(X2, Y2)), A),
        B,
      );
    ...
    toAffine(invertedZ) {
      ...
      if (iz != null && typeof iz !== "bigint")
        throw new TypeError(
          '"invertedZ" expected bigint, got type=' + typeof iz,
        );
      const { X, Y, Z } = p;
      const is0 = p.is0();
...
export function eddsa(Point, cHash, eddsaOpts = {}) {
  ...
  if (outputLen !== undefined) {
    ...
    if (outputLen !== expectedLen)
      throw new Error(
        `hash.outputLen must be ${expectedLen}, got ${outputLen}`,
      );
  }
  const randomBytes =
    opts.randomBytes === undefined ? wcRandomBytes : opts.randomBytes;
  ...
  function hashDomainToScalar(context = Uint8Array.of(), ...msgs) {
    ...
    return modN_LE(
      hash(domain(msg, abytes(context, undefined, "context"), !!prehash)),
    );
  }
  ...
  function verify(sig, msg, publicKey, options = verifyOpts) {
    ...
    const zip215 =
      options.zip215 === undefined ? !!verifyOpts.zip215 : options.zip215;
  ...
  function isValidPublicKey(key, zip215) {
    try {
      ...
      return !!Point.fromBytes(
        key,
        zip215 === undefined ? verifyOpts.zip215 : zip215,
      );
    } catch (error) {
      return false;
    }
  ...
  const utils = {
    ...
    toMontgomery(publicKey) {
      if (toMontgomery === undefined)
        throw new Error(
          "Montgomery conversion is not supported for this curve",
        );
