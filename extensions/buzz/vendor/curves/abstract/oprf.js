...
export function createOPRF(opts) {
  ...
  function getTranscripts(B, C, D, ctx) {
    ...
    for (let i = 0; i < C.length; i++) {
      ...
      const di = hashToScalarPrefixed(
        encode(seed, i, Ci, Di, "Composite"),
        ctx,
      );
      res.push(di);
    }
  ...
  function verifyProof(ctx, B, C, D, proof) {
    ...
    const [c, s] = [proof.subarray(0, Fn.BYTES), proof.subarray(Fn.BYTES)].map(
      (f) => Fn.fromBytes(f),
    );
  ...
  const poprf = (info) => {
    ...
    return Object.freeze({
      ...
      finalizeBatch(items, proof, tweakedKey) {
        ...
        const evalPoints = items.map((i) =>
          wirePoint("evaluated", i.evaluated),
        );
        verifyProof(
          ctxPOPRF,
