...
export function weierstrass(params, extraOpts = {}) {
  ...
  const randomBytes =
    extraOpts.randomBytes === undefined ? wcRandomBytes : extraOpts.randomBytes;
  if (endo) {
    if (
      !Fp.is0(CURVE.a) ||
  ...
  const encodePoint =
    extraOpts.toBytes === undefined ? pointToBytes : extraOpts.toBytes;
  const decodePoint =
    extraOpts.fromBytes === undefined ? pointFromBytes : extraOpts.fromBytes;
  ...
  class Point {
    ...
    static fromBytes(bytes) {
      const P = Point.fromAffine(
        decodePoint(abytes(bytes, undefined, "point")),
      );
      P.assertValidity();
      return P;
    }
...
export function ecdh(Point, ecdhOpts = {}) {
  ...
  const { Fn } = Point;
  const randomBytes_ =
    ecdhOpts.randomBytes === undefined ? wcRandomBytes : ecdhOpts.randomBytes;
...
export function ecdsa(Point, hash, ecdsaOpts = {}) {
  ...
  const randomBytes =
    opts.randomBytes === undefined ? wcRandomBytes : opts.randomBytes;
  const hmac =
    opts.hmac === undefined
  ...
  function validateMsgAndHash(message, prehash) {
    ...
    return prehash
      ? abytes(hash_(message), undefined, "prehashed message")
      : message;
  }
  /**
  ...
  function prepSig(message, secretKey, opts) {
    ...
    function k2sig(kBytes) {
      ...
      if (csprng !== undefined) {
        ...
        const b = bytesToNumberBE(
          mapHashToField(csprng(blindLength), CURVE_ORDER),
        );
      ...
      return new Signature(
        r,
        normS,
        hasLargeRecoveryLifts ? undefined : recovery,
      );
    }
    return { seed, k2sig };
  }
  ...
  function verify(signature, message, publicKey, opts = {}) {
    ...
    try {
      ...
      if (
        format === "recovered" &&
        sig.recovery !== getRecoveryBit(q.x, q.y, r)
      )
        return false;
      return true;
    } catch (e) {
