...
const validateSigners = (signers, title = "signers") => {
  ...
  if (signers.min < 2 || signers.max < 2 || signers.min > signers.max)
    throw new Error(
      "Wrong signers info: min=" + signers.min + " max=" + signers.max,
    );
};
...
export function createFROST(opts) {
  ...
  const hashToScalar =
    opts.hashToScalar === undefined
      ...
      : opts.hashToScalar;
  const H1Prefix = utf8ToBytes(
    opts.H1 !== undefined ? opts.H1 : opts.name + "rho",
  );
  const H2Prefix = utf8ToBytes(
    opts.H2 !== undefined ? opts.H2 : opts.name + "chal",
  );
  const H3Prefix = utf8ToBytes(
    opts.H3 !== undefined ? opts.H3 : opts.name + "nonce",
  );
  const H4Prefix = utf8ToBytes(
    opts.H4 !== undefined ? opts.H4 : opts.name + "msg",
  );
  const H5Prefix = utf8ToBytes(
    opts.H5 !== undefined ? opts.H5 : opts.name + "com",
  );
  const HDKGPrefix = utf8ToBytes(
    opts.HDKG !== undefined ? opts.HDKG : opts.name + "dkg",
  );
  const HIDPrefix = utf8ToBytes(
    opts.HID !== undefined ? opts.HID : opts.name + "id",
  );
  ...
  const randomScalar = (rng = randomBytes) => {
    ...
    const t = mapHashToField(
      rng(getMinHashLength(Fn.ORDER)),
      Fn.ORDER,
      Fn.isLE,
    );
  ...
  const adjustPoint =
    opts.adjustPoint === undefined ? (n) => n : opts.adjustPoint;
  ...
  const serializeIdentifier = (id) =>
    bytesToHex(Fn.toBytes(validateIdentifier(id)));
  ...
  const generateSecretPolynomial = (
    ...
    const secretScalar =
      secret === undefined ? randomScalar(rng) : Fn.fromBytes(secret);
    if (!coeffs) {
      coeffs = [];
  ...
  const ProofOfKnowledge = {
    challenge: (id, verKey, R) =>
      HDKG(
        concatBytes(Fn.toBytes(id), serializePoint(verKey), serializePoint(R)),
      ),
    ...
    validate(id, commitment, proof) {
      ...
      if (
        !R.equals(Point.BASE.multiplyUnsafe(z).subtract(phi.multiplyUnsafe(c)))
      )
  ...
  const validateSecretShare = (identifier, commitment, signingShare) => {
    ...
    if (
      !Point.BASE.multiply(signingShare).equals(
        evalutateVSS(identifier, commitment),
      )
    )
  ...
  const generateNonce = (secret, rng = randomBytes) =>
  ...
  const frost = {
    ...
    DKG: Object.freeze({
      ...
      round1: (id, signers, secret, rng = randomBytes) => {
        ...
        const proofOfKnowledge = ProofOfKnowledge.compute(
          idNum,
          coefficients,
          commitment,
          rng,
      ...
      round2: (secret, others) => {
        ...
        if (secret.round2Cache !== undefined) return secret.round2Cache;
        const res = {};
        for (const p of others) {
          ...
          const signingShare = Fn.toBytes(
            polynomialEvaluate(id, secret.coefficients),
          );
          res[p.identifier] = {
      ...
      round3: (secret, round1, round2) => {
        ...
        for (const r2 of round2) {
          ...
          if (!merged[r2.identifier])
            throw new Error(
              "round1 share for " + r2.identifier + " is missing",
            );
        ...
        const localShare = polynomialEvaluate(
          secret.identifier,
          secret.coefficients,
        );
        ...
        for (const k in commitments)
          verifyingShares[k] = serializePoint(
            evalutateVSS(parseIdentifier(k), mergedCommitment),
          );
    ...
    validateSecret(secret, pub) {
      validateObject(
        secret,
        { identifier: "string", signingShare: "object" },
        {},
        "secret",
      );
    ...
    signShare(secret, pub, nonces, commitmentList, msg) {
      ...
      const commitment = commitmentList.find(
        (i) => i.identifier === secret.identifier,
      );
      if (!commitment) throw new Error("missing signer commitment");
      if (
        bytesToHex(commitment.hiding) !==
          bytesToHex(expectedCommitment.hiding) ||
        bytesToHex(commitment.binding) !==
          bytesToHex(expectedCommitment.binding)
      )
      ...
      const { lambda, challenge, bindingFactor, groupCommitment } =
        prepareShare(
          pub.commitments[0],
          commitmentList,
          msg,
      ...
      const N = opts.adjustNonces
        ? opts.adjustNonces(groupCommitment, nonces)
        : nonces;
      const hidingNonce = opts.adjustNonces
        ? Fn.fromBytes(N.hiding)
        : hidingNonce0;
      const bindingNonce = opts.adjustNonces
        ? Fn.fromBytes(N.binding)
        : bindingNonce0;
    ...
    verifyShare(pub, commitmentList, msg, identifier, sigShare) {
      ...
      const { lambda, challenge, bindingFactor, groupCommitment } =
        prepareShare(pub.commitments[0], commitmentList, msg, identifier);
    ...
    utils: Object.freeze({
      ...
      randomScalar: (rng = randomBytes) =>
        Fn.toBytes(genPointScalarPair(rng).scalar),
