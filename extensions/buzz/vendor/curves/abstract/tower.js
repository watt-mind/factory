...
function calcFrobeniusCoefficients(
  ...
  if (typeof modulus !== "bigint" || modulus <= _1n)
    throw new Error(
      "calcFrobeniusCoefficients: expected valid modulus, got " + modulus,
    );
  if (degree <= 0)
    throw new Error(
      "calcFrobeniusCoefficients: expected positive degree, got " + degree,
    );
  if (num <= 0)
    throw new Error(
      "calcFrobeniusCoefficients: expected positive row count, got " + num,
    );
  if (divisorN <= 0)
    throw new Error(
      "calcFrobeniusCoefficients: expected positive divisor, got " + divisorN,
    );
...
export function psiFrobenius(Fp, Fp2, base) {
  ...
  const mapAffine = (fn) => (c, P) => {
    if (typeof c !== "function")
      throw new TypeError(
        '"c" expected point constructor, got type=' + typeof c,
      );
    validatePointCons(c);
    if (!(P instanceof c))
...
class _Field2 {
  ...
  isValid(num) {
    if (!isObj(num))
      throw new TypeError(
        "invalid field element: expected object, got " + typeof num,
      );
    const { c0, c1 } = num;
    const { Fp } = this;
...
class _Field6 {
  ...
  get FROBENIUS_COEFFICIENTS_1() {
    ...
    const { Fp2 } = this;
    const { Fp } = Fp2;
    const rows = calcFrobeniusCoefficients(
      Fp2,
      Fp2.NONRESIDUE,
      Fp.ORDER,
      6,
