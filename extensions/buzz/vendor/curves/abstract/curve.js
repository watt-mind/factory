...
export function validatePointCons(Point) {
  ...
  if (typeof pc !== "function")
    throw new TypeError(
      '"Point" expected constructor, got type=' + typeof Point,
    );
...
function validateW(W, bits, min = 1) {
  if (!Number.isSafeInteger(W) || W < min || W > bits)
    throw new Error(
      "invalid window size, expected [" + min + ".." + bits + "], got W=" + W,
    );
}
...
function validateMSMScalars(scalars, field, maxScalar) {
  ...
  scalars.forEach((s, i) => {
    const ok =
      maxScalar === undefined ? field.isValid(s) : isPosBig(s) && s < maxScalar;
...
export class ScalarMultiplier {
  ...
  assertPoint(point) {
    if (!(point instanceof this.Point))
      throw new TypeError(
        '"point" expected Point instance, got type=' + typeof point,
      );
  }
  ...
  runCT(point, n, bits, transform) {
    ...
    return this.wnafCachedCT(
      this.getWnafPrecomputes(W, point, bits, transform),
      n,
    );
  }
  ...
  fixedWindowCT(point, n, bits) {
    ...
    for (let window = windows - 1; window >= 0; window--) {
      ...
      if (window !== windows - 1)
        for (let d = 0; d < W; d++) acc = acc.double();
...
export function mulAddUnsafe(c, points, scalars, allowOversized = false) {
  ...
  validateMSMScalars(
    scalars,
    c.Fn,
    allowOversized ? c.Fn.ORDER ** _4n : undefined,
  );
...
export function interleavedMSMUnsafe(c, points, windowSize) {
  ...
  return (scalars) => {
    ...
    if (scalars.length > points.length)
      throw new Error(
        "array of scalars must not be larger than array of points",
      );
    return wnafWalk(
      c.ZERO,
      tables,
