...
function checkU32(n, title = "n") {
  ...
  if (typeof n !== "number")
    throw new TypeError(
      `wrong u32 integer "${title}": expected number, got type=${typeof n}`,
    );
  if (!Number.isSafeInteger(n) || n < 0 || n > 0xffffffff)
    throw new RangeError(
      `wrong u32 integer "${title}": expected 0..4294967295, got ${n}`,
    );
  return n;
}
/**
...
export function bitReversalInplace(values) {
  if (
    ...
    throw new TypeError(
      '"values" expected array-like, got type=' + typeof values,
    );
...
export function rootsOfUnity(field, generator) {
  ...
  if (generator !== undefined && typeof generator !== "bigint")
    throw new TypeError(
      '"generator" expected bigint, got type=' + typeof generator,
    );
...
export const FFTCore = (F, coreOpts) => {
  ...
  if (skipStages > maxSkipStages)
    throw new Error(
      `FFT: wrong skipStages: expected 0 <= skipStages <= ${maxSkipStages}`,
    );
  // Wrong-sized root tables can stay in-bounds for some loop shapes and silently compute nonsense.
  if (roots.length !== N)
    throw new Error(
      `FFT: wrong roots length: expected ${N}, got ${roots.length}`,
    );
...
export function FFT(roots, opts) {
  ...
  const loopKey = (bits, isInverse, brpInput, brpOutput) =>
    (bits << 3) |
    (isInverse ? 4 : 0) |
    (brpInput ? 2 : 0) |
    (brpOutput ? 1 : 0);
...
export function poly(field, roots, create, fft, length) {
  ...
  const checkPoly = (title, value) => {
    if (!isPoly(value))
      throw new TypeError(
        `"${title}" expected polynomial, got type=${typeof value}`,
      );
  };
  ...
  return {
    ...
    shift(p, factor) {
      ...
      if (length !== undefined && p.length !== length)
        throw new Error(
          `poly: expected fixed length ${length}, got ${p.length}`,
        );
    ...
    lagrange: {
      basis: (x, n, brp = false, weights) => {
        if (!isPowerOfTwo(n))
          throw new Error(
            "poly.lagrange: expected power of two length, got " + n,
          );
        const bits = log2(n);
    ...
    vanishing(roots) {
      ...
      if (length !== undefined && roots.length !== length)
        throw new Error(
          `poly: expected fixed length ${length}, got ${roots.length}`,
        );
