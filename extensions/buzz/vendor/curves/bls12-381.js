...
function bls12RecoverG0(g1, { g2, g3, g4, g5 }) {
  ...
  const t = Fp2.add(
    Fp2.sub(Fp2.mul(Fp2.sub(Fp2.sqr(g1), g3g4), _2n), g3g4),
    Fp2.mul(g2, g5),
  );
...
function bls12CyclotomicExpCompressed(num, squarings) {
  ...
  return {
    result: Fp12.mul(Fp12.mul(elems[0], elems[1]), elems[2]),
    last: elems[2],
  };
}
...
let frob;
const getFrob = () =>
  frob || (frob = psiFrobenius(Fp, Fp2, Fp2.div(Fp2.ONE, Fp2.NONRESIDUE)));
