 ...
 * ```
 */
export const secp256k1_hasher = /* @__PURE__ */ (() =>
  createHasher(
    Pointk1,
    (scalars) => {
 ...
 * ```
 */
export const secp256k1_FROST = /* @__PURE__ */ (() =>
  createFROST({
...
function frostPubToEvenY(pub) {
  ...
  return {
    ...
    commitments: pub.commitments.map((i) =>
      Pointk1.fromBytes(i).negate().toBytes(),
    ),
...
function frostNoncesToEvenY(groupCommitment, nonces) {
  if (hasEven(groupCommitment.y)) return nonces;
  const Fn = Pointk1.Fn;
  return {
...
function frostTweakSecret(s, pub, merkleRoot) {
  ...
  const signingShare = Fn.toBytes(
    Fn.add(Fn.fromBytes(keyPackage.signingShare), t),
  );
  return {
...
function frostTweakPublic(pub, merkleRoot) {
  ...
  for (const k in PKPackage.verifyingShares) {
    verifyingShares[k] = Pointk1.fromBytes(PKPackage.verifyingShares[k])
      .add(tp)
      .toBytes();
  }
  return {
 ...
 * ```
 */
export const schnorr_FROST = /* @__PURE__ */ (() =>
  createFROST({
    ...
    parsePublicKey(publicKey) {
      ...
      throw new Error(
        `expected x-only or compressed public key, got length=${publicKey.length}`,
      );
    },
    adjustScalar(n) {
    ...
    adjustGroupCommitmentShare: (GC, GCShare) =>
      !hasEven(GC.y) ? GCShare.negate() : GCShare,
