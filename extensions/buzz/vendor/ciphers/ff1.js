...
import { aarray, abytes, anumber, bytesToNumberBE, clean, numberToBytesBE, } from "./utils.js";
...
export function BinaryFF1(key, tweak = EMPTY_BUF) {
    ...
    return {
        encrypt: (x) => {
            if (Array.isArray(x)) {
                aarray(x, 'x', (elm, title) => {
                    anumber(elm, title);
                    if (elm > 255)
                        throw new RangeError(`"${title}" expected byte`);
                });
            }
            else {
                x = abytes(x, undefined, 'x');
            }
            return binLE.decode(ff1.encrypt(binLE.encode(x)));
        },
        decrypt: (x) => {
            if (Array.isArray(x)) {
                aarray(x, 'x', (elm, title) => {
            ...
            return binLE.decode(ff1.decrypt(binLE.encode(x)));
        },
    };
}
