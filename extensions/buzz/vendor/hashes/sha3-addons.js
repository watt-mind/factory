...
function cshakePers(h, opts = {}) {
  ...
  const fn =
    opts.NISTfn === undefined ? EMPTY_BUFFER : kdfInputToBytes(opts.NISTfn);
...
const gencShake = (suffix, blockLen, outputLen) =>
  createHasher((opts = {}) =>
    cshakePers(
...
export class _TupleHash extends Keccak {
  ...
  update(data) {
    if (!this.tupleMode) return super.update(data);
    abytes(data);
...
export class _ParallelHash extends Keccak {
  ...
  update(data) {
    if (!this.leafMode) return super.update(data);
    aexists(this);
    abytes(data);
...
function genPrl(blockLen, outputLen, leaf, xof = false) {
  const parallel = (message, opts) =>
    parallel.create(opts).update(message).digest();
  parallel.create = (opts = {}) =>
    new _ParallelHash(
      blockLen,
 ...
 * ```
 */
export const parallelhash128xof = /* @__PURE__ */ genPrl(
  168,
  16,
  cshake128,
  true,
 ...
 * ```
 */
export const parallelhash256xof = /* @__PURE__ */ genPrl(
  136,
  32,
  cshake256,
  true,
);
const genTurbo = (blockLen, outputLen) =>
  createHasher((opts = {}) => {
    opts = checkOpts({}, opts);
    ...
    if (!Number.isSafeInteger(D) || D < 0x01 || D > 0x7f)
      throw new Error(
        '"D" (domain separation byte) must be 0x01..0x7f, got: ' + D,
      );
...
export class _KangarooTwelve extends Keccak {
  ...
  // and silently miss the tree state (leaf hash, chunk counters).
  _cloneInto(to) {
    const { blockLen, leafLen, leafHash, outputLen, personalization, rounds } =
      this;
    ...
    to ||= new _KangarooTwelve(blockLen, leafLen, outputLen, rounds, {
      personalization:
        personalization === EMPTY_BUFFER ? undefined : personalization,
    });
    super._cloneInto(to);
    ...
    if (!fresh) {
      ...
      if (personalization === EMPTY_BUFFER) {
        ...
        old === EMPTY_BUFFER ||
        old.length !== personalization.length
      ) {
 ...
 * ```
 */
export const kt128 = /* @__PURE__ */ createHasher(
  (opts = {}) => new _KangarooTwelve(168, 32, chooseLen(opts, 32), 12, opts),
);
/**
 ...
 * ```
 */
export const kt256 = /* @__PURE__ */ createHasher(
  (opts = {}) => new _KangarooTwelve(136, 64, chooseLen(opts, 64), 12, opts),
);
const genHopMAC = (hash) => {
  const kt = hash;
  return (key, message, personalization, dkLen) =>
    kt(key, { personalization: kt(message, { personalization }), dkLen });
};
/**
