...
function argon2Init(password, salt, type, opts) {
  ...
  if (memUsed > maxmem)
    throw new Error(
      '"maxmem" limit was hit: memUsed(mP*1024)=' +
        memUsed +
        ", maxmem=" +
        maxmem,
    );
...
function* argon2Blocks(ctx) {
  const { type, mP, p, t, version, B, laneLen, lanes, segmentLen, perBlock } =
    ctx;
  ...
  for (let r = 0; r < t; r++) {
    ...
    for (let s = 0; s < ARGON2_SYNC_POINTS; s++) {
      ...
      const dataIndependent =
        type == AT.Argon2i || (type == AT.Argon2id && r === 0 && s < 2);
      for (let l = 0; l < p; l++) {
        address[256 + 2] = l;
        address[256 + 12] = 0;
        let startPos = 0;
 ...
 * ```
 */
export const argon2d = (password, salt, opts) =>
  argon2(AT.Argon2d, password, salt, opts);
/**
 ...
 * ```
 */
export const argon2dAsync = (password, salt, opts) =>
  argon2Async(AT.Argon2d, password, salt, opts);
/**
 ...
 * ```
 */
export const argon2iAsync = (password, salt, opts) =>
