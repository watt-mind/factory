...
3. Spawn a subagent (general-purpose, run in background). **When the parent run is Claude Code — identified by `CLAUDECODE` being set in the environment — pass `model: opus`** — implementation quality is the product while the orchestration stays on the cheaper session model. **For every other parent harness, omit the model override** so the subagent inherits that harness's configured default. Give it a **self-contained prompt** containing the full ticket (ID, description, all five sections), the worktree path and its ports/database, and these standing orders:
   ...
   - On success: push the branch, `gh pr create --title "..." --body "Fixes <ISSUE-ID>"` (append `\n\nrun:$FACTORY_RUN_ID` to the body when `$FACTORY_RUN_ID` is set, and omit it in interactive sessions when `$FACTORY_RUN_ID` is unset), post the mandatory structured **`## Handoff` comment** in the exact format `/factory-ticket` step 6 defines (PR link, verification command + result, UX critique verdict, files vs `Owned Paths`, risks) as a mandatory prerequisite before moving the ticket to `In Review` + `ai:needs-review` (remove `ai:in-progress`), and report terminal state `STATE: PR_OPEN`.
     - When the UX critique captured screenshots, declare each retained workspace-relative file in the dispatch `result.json` as `{"kind":"ux-screenshot","path":"ux-screenshots/<filename>.png"}`. Factory then hashes and publishes it through the control API's `GET /artifacts/<sha256>` endpoint. Compute each file's SHA-256 before writing the PR evidence, and use the browser-facing artifact URL `$FACTORY_WEB_URL/api/artifacts/<sha256>` (the web proxy for that endpoint). Do not use `file://`, a temporary browser path, a repository URL, or an unverified guessed hash.
     - Add a dedicated PR comment headed `## 📸 UX Review & Screenshots` after the artifact declarations are prepared. For every retained screenshot, include an embedded Markdown image and a labelled artifact link, for example `![Mobile upload list]($FACTORY_WEB_URL/api/artifacts/<sha256>)` followed by `[Open immutable artifact]($FACTORY_WEB_URL/api/artifacts/<sha256>)`. State what each image demonstrates and the critic verdict. This keeps visual evidence on the GitHub PR rather than in Linear; screenshots are never committed to the repo.
     - On the same PR, run `gh pr edit <PR> --add-label has-screenshots --add-label area:ui-ux`. `has-screenshots` and `area:ui-ux` are provisioned by `factory init --control-plane github`; do not silently ignore a label failure. Apply neither label when no screenshot was captured.
