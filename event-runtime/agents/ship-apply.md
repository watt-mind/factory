...
| `merge_rc_pr` | probe both pins, find the head commit's run with `gh run list`, wait via REST using `gh run watch <run-id> --exit-status --interval 60` (never sleep-and-poll), then `gh pr merge --merge` |
